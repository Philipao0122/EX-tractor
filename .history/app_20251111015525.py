...
# Configure temp directory at the module level
app_root = os.path.dirname(os.path.abspath(__file__))
temp_dir = os.path.join(app_root, 'temp')
os.makedirs(temp_dir, exist_ok=True)

# Clean up old files on startup (optional, remove if you want to keep files between restarts)
...
# Temp directory is now configured at the top of the file with the download route
...
# temp_dir is now defined at the top of the file
...
@app.route('/api/fetch-thumbnails', methods=['POST', 'OPTIONS'])
@cross_origin()
def fetch_thumbnails():
    ...
    try:
        data = request.get_json()
        username_or_url = data.get('username_or_url')
        
        if not username_or_url:
            return jsonify({'error': 'Missing username_or_url parameter'}), 400
        
        # Extract username from URL if a full URL was provided
        if 'instagram.com/' in username_or_url:
            # Handle different Instagram URL formats
            if '/p/' in username_or_url:  # Post URL
                # For post URLs, we can't fetch thumbnails directly
                return jsonify({
                    'error': 'Direct post URLs are not supported. Please provide a profile username.'
                }), 400
            else:  # Profile URL
                username = username_or_url.split('instagram.com/')[1].split('/')[0].split('?')[0]
        else:
            username = username_or_url.strip('@')
        
        logger.info(f'Fetching thumbnails for username: {username}')
        
        # Call Instagram API
        headers = {
            'X-RapidAPI-Key': API_KEY,
            'X-RapidAPI-Host': 'instagram-scraper-20231.p.rapidapi'
        }
        ...
        if not thumbnail_urls:
            return jsonify({
                'error': 'No thumbnails found for this profile',
                'thumbnails': []
            })
        
        # Save thumbnails to temp directory and create local URLs
        saved_thumbnails = []
        for i, url in enumerate(thumbnail_urls[:10]):  # Limit to first 10 thumbnails
            try:
                ...
                # Save to temp file
                filename = f'thumb_{username}_{i}.jpg'
                filepath = os.path.join(temp_dir, filename)
                
                with open(filepath, 'wb') as f:
                    shutil.copyfileobj(response.raw, f)
                
                # Create URL for the saved thumbnail
                scheme = request.headers.get('X-Forwarded-Proto', request.scheme)
                thumbnail_url = f'{scheme}://{request.host}/download/{filename}'
                saved_thumbnails.append(thumbnail_url)
        ...
        return jsonify({
            'error': f'Error fetching from Instagram API: {str(e)}',
            'thumbnails': []
        }), 500
    except Exception as e:
...
def download_file(filename):
    try:
        ...
        # Use send_file with proper caching headers
        response = send_file(
            file_path,
            mimetype=mime_type,
