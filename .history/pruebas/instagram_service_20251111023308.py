...
def fetch_eltiempo_thumbnails(ignore_rate_limit=False):
    """
    Automatically fetch and save thumbnails for eltiempo Instagram account.
    
    Args:
        ignore_rate_limit (bool): If True, will ignore rate limiting (useful for server startup)
    """
    username = "eltiempo"
    
    # Skip rate limit check if explicitly requested (e.g., on server startup)
    if not ignore_rate_limit and not can_fetch_thumbnails(username):
...
if __name__ == '__main__':
    ...
    # Fetch eltiempo thumbnails when server starts (ignore rate limit)
    fetch_eltiempo_thumbnails(ignore_rate_limit=True)
